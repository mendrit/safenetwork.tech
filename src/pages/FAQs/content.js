      ...
      items: [
        {
          ...
          answer: [
            'SafeNetwork je tvořena uzly zvanými Vault. Vault je program, který běží na počítači, které propojuje zařízení se sítí.  Vaulty společně spravují datová uložiště v síti, přesouvají kousky šifrovaných dat od uživatelů po síti. Žádný uživatel (farmář) nikdy nemůže svévolně dešifrovat data uložená na svém Vaultu, ale na oplátku, bude za poskytnutí prostoru a kapacity svého počítače síti odměněn v kryptoměně SafeCoin.',
            'Vault zajistí , aby události na síti byly validní. Vaulty jsou řazeny do skupin, které zodpovědně dohlíží na uložení dat v dané sekci (určitý blok adres).',
            'Tyto skupiny uzlů se slučují, či dělí, bez lidského zásahu, díky autonomní síti SafeNetwork, která má kompletní kontrolu nad tímto procesem. Ve stejném smyslu, jsou šifrovaná data posílána po síti plně autonomně. Žádné centrální servery nebo prostředníci (jakým jsou třeba BitTorrent trackery) nejsou v síti potřeba. Žádný centrální úřad nedohlíží na tento proces.',
            'Stejně jako děti nemají právo hlasovat ve volbách, Vault nemá možnost hlasovat o různých událostech v síti (jako přijetí nového uzlu, nebo uložení části dat), dokud se neprokáže, že je spolehlivý. Na počátku musí každý Vault úspěšně dokončit  [Důkaz práce/PoR](https://www.youtube.com/watch?v=rdczpOlLaVk) k přihlášení se do sítě, aby potvrdil, že může poskytnout určitou část kapacity CPU a internetového připojení. Pak je zařazen do určité sekce a dostane nízkou hodnotu „stáří uzlu". Toto je měřítkem důvěryhodnosti. Po prvotním připojení, bude síť náhodně automaticky tento vault přesouvat ze sekce do sekce, kde při každé této příležitosti, si může zvýšit svou reputaci (Stáří uzlu). V momentě, kdy hodnota „stáří uzlu" dosáhne určité hodnoty, může se stát aktivním účastníkem při skupinovém hlasování. Vaulty s nějvětší hodnotou „stáří uzlu" v dané sekci jsou pojmenování jako starci/Elder. Důsledkem je, že noví vaulti musí nejdříve prokázat svou cenu a zodpovědnost v různých sekcích, než se mohou stát starci a získat tak možnost hlasovat. To umožní ochránit každou sekci v SafeNetwork a snížit šance útočníka na pravděpodobnost blízkou nule.',
            'Vaulty také kryptograficky kontrolují zprávy a přebírají na sebe další jasně vymezené role, zvané osobnosti/personas. Každý vault bude mít klienta se správou osobnosti. To uchová údaje o detailech účtu pro každého klienta v rámci dané sekci. Například potvrdí, kolik dat bylo nahráno na SafeNetwork, kolik dat bylo uloženo a zůstatek SafeCoinů potřebný pro další nahrávání dat na síť. Zatímco správce klienta bude znát zůstatek na účtu, nemá další možnosti, jak je propojit s vlastníkem (jako např. IP adresa, přezdívka, jméno a příjmení, ...).',
            'Každý vault má také osobního správce dat. Ten spravuje, kde jsou šifrované kousky dat dalších uživatelů uchovány a má za ně zodpovědnost ve své sekci.'
          ]
        },
        {
          question: 'Proč potřebujeme autonomní síť?',
          answer: [
            'Autonomní síť je taková, která spravuje tvá veškerá data a komunikaci, bez jakéhokoli lidského vměšování a žádných prostředníků. Je to síť, která se spravuje samostatně. Zdroje nepocházejí od centralizovaných IT správců, což redukuje jakékoli možnosti zákeřných aktivit, nebo těch, kvůli nedbalé správě. Síť nevyžaduje ničí povolení, zapojit se může každý, kdo síť vyhledá, čímž se odstraní riziko monopolizace nějakým subjektem, kterému by jeho vliv mohl jinak nekontrolovaně růst.',
            'Spousta velkých úniků dat vzniklo lidskou chybou, ale tento problém jde do větší hloubky. Lidé se stávají čím dál více zavislí na systémech, které uchovávají jejich data. Čím dál více těchto osobních dat, je v rukou třetích stran a tím více roste každým dnem riziko dalšího úniku, nebo zneužití dat. Každý už byl svědkem toho, jak své osobní data byla použita pro takové účely, se kterými by určitě nesouhlasil. Jaká je šance, že tvůj přístup ke svým datům ti bude jednoho dne naprosto odříznut? Nebo ztratíš kontrolu nad proudem dat o svém životě? V některých státech je to již nyní běžná realita.',
            'Můžeme vylepšit fyzickou zálohu dat. Zajistíme, aby data nemohla být smazána, přepsána, pozměněna třeba i za úplatek, nebo zpřístupněna, bez povolení vlastníka. A jen díky kompletnímu odstranění lidských správců dat, můžeme tuto fyzickou bezpečnost zajistit. Je potřeba mít uložena data na místech, o kterých ví pouze samotná síť a ve které nelze uživatele identifikovat. Pouze autonomní síť poskytuje takovou úroveň bezpečnosti.',
            'Autonomní síť automaticky rozdělí a zašifruje (pomocí vlastního šifrování) všechny data ještě předtím, než je dynamicky uloží do lokací, které si vybere. Uzly se připojují anonymně a síť je bude pravidelně přesouvat mezi skupinami a to opět bez jakýkoliv lidských zásahů, nebo centralizovaných záznamů. Celá skupina uzlů dělá společná rozhodnutí na základě zpráv, které obdrží.',
            'Autonomní síť také umožňuje, vytvořit dodatečné kopie populárních dat, což urychlí vyřízení všech jejich požadavků. A zároveň, síť umí rozpoznat duplikované kopie stejných dat a redukovat je na minimum.',
            'Náš design se inspiroval prostými mravenci. Mravenčí kolonie se vyjímají spletitým a vysoce organizovaným chováním v masivním měřítku, bez závislosti na centrální autoritě. Díky ní, každý mravenec plní různé povinosti na základě toho, co jeho kolonie zrovna potřebuje. V podobném duchu, uzly v síti SafeNetwork plní rozližné funkce, dle toho, s jakými druhy zpráv se zrovna potřebují vypořádat.',
            'Takže přoč tedy potřebujeme autonomní síť? Protože lidé dělají chyby a centralizované uložiště jsou náchylná k chybám. Proto potřebujeme společně vybudovat platformu, na které může lidstvo spolupracovat na cestě do budoucnosti.'
          ]
        },
        {
